...
public class GestionTransaccionController {
    ...
    public void enviarTransaccion(String idTransaccion, LocalDate fecha, TipoTransaccion tipoTransaccion, Double monto, String descripcion, CuentaDto cuentaOrigen, CuentaDto cuentaDestino) {
        try (Socket socket = new Socket(SERVER_HOST, SERVER_PORT);
            ...
            if (response) {
                persistencia.guardarRegistroLog("ClickGuardarTransaccion", 1,"Se ha guardado una transaccion exitosamente "+usuarioActual.nombre());
            } else {
                persistencia.guardarRegistroLog("ClickGuardarTransaccion", 3,"Error al guardar una transaccion  "+usuarioActual.nombre());
            }
