...
public class AdicionPresupuestosViewController {
    ...
    void ClickAgregarPresupuesto(ActionEvent event) {
        ...
        if (resultado) {
            ...
            persistencia.guardarRegistroLog("ClickAgregarPresupuesto",1,"[Administrador] Se ha guardado un presupuesto exitosamente ");
            limpiarCampos();
        } else {
            persistencia.guardarRegistroLog("ClickAgregarPresupuesto",2,"[Administrador] No se ha podido guardar un presupuesto ");
            mostrarMensaje("Error", "Error en la Adición", "No se pudo añadir el presupuesto. Verifique los datos o la conexión al servidor.", Alert.AlertType.ERROR);
        }
    }
